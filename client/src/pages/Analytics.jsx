...
export default function Analytics() {
  ...
  if (!data) return <Layout><div className="text-center py-16 text-gray-400">Failed to load analytics</div></Layout>
  ...
  return (
    <Layout>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Complaint trends, priority distribution, and diagnostic patterns</p>
      </div>

      {/* Summary metrics */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        <MetricCard label="Total Complaints" value={summary.total} />
        <MetricCard label="Open" value={summary.open} color="text-yellow-600" />
        <MetricCard label="Escalated" value={summary.escalated} color="text-red-600" />
        <MetricCard label="Avg Resolution" value={summary.avg_resolution_hours != null ? `${summary.avg_resolution_hours}h` : 'N/A'} sub="hours to resolve" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        {/* Device breakdown */}
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">Complaints by Device</h3>
          {by_device.length ? (
            <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="card p-5 mb-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">30-Day Submission Trend</h3>
        {trend_30d.length ? (
      ...
      {/* Lab type breakdown */}
      <div className="card p-5">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">Complaints by Lab Type</h3>
        {by_lab_type.length ? (
